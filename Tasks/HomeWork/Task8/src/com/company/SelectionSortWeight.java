...
public class SelectionSortWeight implements HumanSorter {

    public SelectionSortWeight (Human humans[]/*, HumanComparator comparator*/){
       // this.humans = humans;
...
*/
  ...
  public void sort ( Human humans[]){
        ...
        for (int i= 0; i < humans.length-1; i++) {
            ...
            for (int j = i+1; j< humans.length; j++) {
                Human b = humans[j];
                HumanComparator comparator = new WeightHumanComparator(minWeightHuman, b);//создаю реализацию AgeHumanComparator интерфейса HumanComparator
